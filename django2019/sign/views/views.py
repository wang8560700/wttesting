...
def login_action(request):
    if request.method=='POST':
        ...
        if user is not  None:
            ...
            return render(request,"index.html",{"erro":"username or password erro! "})
@login_required
...
def sign_index(request, event_id):
    ...
    sign_list=Guest.objects.filter(sign=True,event_id=event_id)
    guest_data = str(len(guest_list))  # 签到人数
    sign_data = str(len(sign_list))    # 已签到人数
